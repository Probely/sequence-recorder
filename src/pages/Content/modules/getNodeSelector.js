// From here: https://github.com/antonmedv/finder
export default function finder(theNode, options) {
  let Limit;
  (function (a) {
    (a[(a.All = 0)] = 'All'),
      (a[(a.Two = 1)] = 'Two'),
  ...
  function* optimize(a, b, c = { counter: 0, visited: new Map() }) {
    if (2 < a.length && a.length > config.optimizedMinLength) {
      for (let d = 1; d < a.length - 1; d++) {
        if (c.counter > config.maxNumberOfTries) {
          return;
        }
        c.counter += 1;
        const e = [...a];
  ...
  if ('html' === theNode.tagName.toLowerCase()) {
    return 'html';
  }
  const c = {
    root: document.body,
  ...
  (config = Object.assign(Object.assign({}, c), options)),
    (rootDocument = findRootDocument(config.root, c));
  let d = bottomUpSearch(theNode, Limit.All, () =>
    bottomUpSearch(theNode, Limit.Two, () => bottomUpSearch(theNode, Limit.One))
  );
  if (d) {
    const b = sort(optimize(d, theNode));
